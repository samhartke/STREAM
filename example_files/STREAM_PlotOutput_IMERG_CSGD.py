...
dt = datetime(2013, 4, 8)  # Define date to start plotting at

# ---- Define the fields that you want to plot:

## Original precipitation input field
prcp = xr.open_dataset('../example_data/IMERG2013.hourly.nc').sel(time=slice(dt.strftime('%Y-%m-%d'),None)).rename({'latitude':'lat','longitude':'lon'}).set_coords(('lat','lon'))
prcp = prcp.transpose('time','lat','lon')['prcp']

## Correlated noise output field
noise = xr.open_dataset('../example_output/noise_output_IMERG_CSGD.nc')['noise'].sel(time=slice(dt.strftime('%Y-%m-%d'),None))

# # Simulated precipitation output field
simPrcp = xr.open_dataset('../example_output/precip_output_IMERG_CSGD.nc')['sim_prcp'].sel(time=slice(dt.strftime('%Y-%m-%d'),None)).transpose('time','lat','lon')
...
for i in range(6):
    ...
    if 'prcp' in globals():
        ...
        plot_map(prcp[i,:,:],ax1,cmap='turbo',vmin=0.,vmax=10.,colorbar=False,left=False,bottom=False)
        if i==0:
    ...
    if 'simPrcp' in globals():
        ...
        plot_map(simPrcp[i,:,:],ax1,cmap='turbo',vmin=0.,vmax=10.,colorbar=False,left=False,bottom=False)
        if i==0:
            ax1.text(-0.08, 0.55, 'Output precip\nfield', va='bottom', ha='center',rotation='vertical',
                 fontsize=20,rotation_mode='anchor',transform=ax1.transAxes,fontweight='bold')

plt.subplots_adjust(hspace=0.25)
fig.savefig('STREAM_output_IMERG_CSGD.jpg',bbox_inches='tight',dpi=1200)
plt.show()
